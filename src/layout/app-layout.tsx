...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiGrid, FiLayers } from 'react-icons/fi';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['developer', 'team-member', 'tester', 'owner'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<ShowcaseInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/showcases` : null),
    () => getShowcases({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Showcase',
      path: isTenantUser ? `/showcases/view/${data?.[0]?.id}` : '/showcases',
      entity: 'showcase',
      service: AccessServiceEnum.PROJECT,
      icon: FiGrid,
    },
    { name: 'Component', path: '/components', entity: 'component', service: AccessServiceEnum.PROJECT, icon: FiLayers },
    {
      name: 'Invitation',
      path: '/invitations',
      entity: 'invitation',
